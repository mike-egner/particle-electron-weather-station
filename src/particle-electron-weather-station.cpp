 ...
 */
/* To do:
     1. compiler instructions to skip over all battery-related functions if not DEVICE_FAMILY == "ELECTRON"
     2. make a cloud function to change external_publishing variable (change from const to variable) + serial input at start?
     3. make a cloud function to change low_power_mode and change that from const + serial input at start?
 */
...
const bool low_power_mode = false;                  //set whether to go into sleep between loops rather than just wait
const unsigned sync_interval = 24 * 60 * 60 * 1000; //clock to sync to Particle cloud once a day
const String channel_id = "373128";
const float rain_bucket_size = 0.2794;
const float anemom_radius_cm = 9;
...
//setup Application Watchdog
ApplicationWatchdog wd(program_speed * 2, System.reset);
...
void updateBuffer(String json_buffer)
{
    ...
    json_buffer = json_buffer + "{\"created_at\":\"" + String(Time.format("%Y-%m-%d %H:%M:%S %z")) + "\",\"field1\":" + String(rainfall, 2) + ",\"field2\":" + String(temp, 2) + ",\"field3\":" + String(humidity, 2) + ",\"field4\":" + String(wind_speed_avg, 2) + ",\"status\":" + String(battery_level) + "},";
...
void powerUp()
{
    ...
    {
        Serial.println("Waiting for Particle cloud connection...");
        delay(1000);
        continue;
    }
    Particle.publish("Connections established.");
#elif (PLATFORM_ID == PLATFORM_PHOTON_DEV)
    WiFi.on();
    WiFi.connect();
    delay(1000);
